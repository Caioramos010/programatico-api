...
@Transactional
class SessaoAtividadeIntegrationTest {
    ...
    @Test
    void fluxoIniciarResponderConcluirDeveCompletarSessao() throws Exception {
        MvcResult inicio = mockMvc.perform(post("/api/aprender/modulos/" + modulo.getId() + "/iniciar")
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.totalExercises").value(1))
                .andReturn();

        JsonNode inicioBody = objectMapper.readTree(inicio.getResponse().getContentAsString());
        long sessionId = inicioBody.get("sessionId").asLong();
        long exercicioId = inicioBody.get("exercises").get(0).get("id").asLong();
        ...
        assertTrue(practiceSessionRepository.findById(sessionId).orElseThrow().getEndedAt() != null);
    }
}
